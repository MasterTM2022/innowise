...
public class CustomLinkedList<E> {
    ...
    public void add(int index, E el) {
        if (index < 0 || index > size) {
            throw new IndexOutOfBoundsException(String.format("Index %d is out of range [0; %d], current CustomLinkedList's size is %d", index, size - 1, size));
        } else {
            if (index == 0) {
                addFirst(el);
            } else if (index == size) {
                ...
                Node<E> newNode = new Node<>(el);
                Node<E> currentNode = findCurrentNodeByIndex(index);
    ...
    private Node<E> findCurrentNodeByIndex(int index) {
        Node<E> currentNode;
        if (index < size / 2) {
            currentNode = first;
